...
import { ClassifiedNumber, LargeNumbersMetaOptions, LargeNumbersOrder } from "./types";
 ...
 */
export const largeNumberPartToWords = (number: number, classifiedNumber: ClassifiedNumber, options?: Partial<LargeNumbersMetaOptions>): string => {

   const lenght = numberLenght(number);

   if(lenght === 1) return toLargeOneDigitNumberWords(number, classifiedNumber.order as LargeNumbersOrder, {...options});
   const words = lenght === 2 ? toLargeTwoDigitNumberWords(number, classifiedNumber.order as LargeNumbersOrder) : toLargeThreeDigitNumberWords(number, classifiedNumber.order as LargeNumbersOrder);
   return words;
}

export const largestNumberOrderToWords = (number: number, reminder: number, order: LargeNumbersOrder) => {
    //TODO: 'edna milijarda' ili milijarda a koga e najgolem del od probojot 
    return toLargeOneDigitNumberWords(number, order, { isLargestNumberOrderPart: reminder === 0 });
}

export const toLargeOneDigitNumberWords = (number: number, order: LargeNumbersOrder, options?:Partial<LargeNumbersMetaOptions>): string => {
...
export const toLargeTwoDigitNumberWords = (number: number, order: LargeNumbersOrder): string => {
...
export const toLargeThreeDigitNumberWords = (number: number, order: LargeNumbersOrder): string => {
